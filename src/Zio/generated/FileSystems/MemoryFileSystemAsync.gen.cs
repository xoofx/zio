...
/// <summary>
/// Async counterpart to <see cref = "MemoryFileSystem"/>.
/// </summary>
[DebuggerDisplay("{" + nameof(DebuggerDisplay) + "(),nq}")]
[DebuggerTypeProxy(typeof(DebuggerProxy))]
public class MemoryFileSystemAsync : FileSystemAsync
{
    ...
    private FileSystemEventDispatcherAsync<Watcher>? _dispatcher;
    /// <summary>
    /// Initializes a new instance of the <see cref = "MemoryFileSystem"/> class.
    /// </summary>
    public MemoryFileSystemAsync()
    {
    ...
    protected MemoryFileSystemAsync(MemoryFileSystemAsync copyFrom)
    {
        if (copyFrom is null)
    ...
    public MemoryFileSystemAsync Clone()
    {
    ...
    protected virtual MemoryFileSystemAsync CloneImpl()
    {
        return new MemoryFileSystemAsync(this);
    }
            ...
            try
            {
                ...
                try
                {
                    ...
                    foreach (var nodePair in directory.Children)
                    {
                        ...
                        var addEntry = (nodePair.Value is FileNode && searchTarget != SearchTarget.Directory && isEntryMatching) || (nodePair.Value is DirectoryNode && searchTarget != SearchTarget.File && isEntryMatching);
    ...
    protected override async IAsyncEnumerable<Zio.FileSystemItemAsync> EnumerateItemsAsyncImpl(UPath path, SearchOption searchOption, SearchPredicateAsync? searchPredicate, [EnumeratorCancellation] CancellationToken cancellationToken)
    {
        var foldersToProcess = new List<UPath>();
        foldersToProcess.Add(path);
        var entries = new List<FileSystemItemAsync>();
        while (foldersToProcess.Count > 0)
        {
            var directoryPath = foldersToProcess[0];
            foldersToProcess.RemoveAt(0);
    ...
    private class Watcher : FileSystemWatcherAsync
    {
        private readonly MemoryFileSystemAsync _fileSystem;
        public Watcher(MemoryFileSystemAsync fileSystem, UPath path) : base(fileSystem, path)
        {
    ...
    private FileSystemEventDispatcherAsync<Watcher> GetOrCreateDispatcher()
    {
        lock (_dispatcherLock)
        {
            _dispatcher ??= new FileSystemEventDispatcherAsync<Watcher>(this);
            return _dispatcher;
        }
    }

    private FileSystemEventDispatcherAsync<Watcher>? TryGetDispatcher()
    {
        lock (_dispatcherLock)
        {
            return _dispatcher;
    ...
    {
        protected readonly MemoryFileSystemAsync FileSystem;
        protected FileSystemNode(MemoryFileSystemAsync fileSystem, DirectoryNode? parentNode, string? name, FileSystemNode? copyNode)
        {
    ...
    {
        private readonly MemoryFileSystemAsync _fs;
        public ListFileSystemNodes(MemoryFileSystemAsync fs)
        {
    ...
    private class DirectoryNode : FileSystemNode
    {
        ...
        public DirectoryNode(MemoryFileSystemAsync fileSystem) : base(fileSystem, null, null, null)
        {
            _children = new Dictionary<string, FileSystemNode>();
            Attributes = FileAttributes.Directory;
        }

        public DirectoryNode(MemoryFileSystemAsync fileSystem, DirectoryNode parent, string name) : base(fileSystem, parent, name, null)
        {
    ...
    {
        public FileNode(MemoryFileSystemAsync fileSystem, DirectoryNode parentNode, string? name, FileNode? copyNode) : base(fileSystem, parentNode, name, copyNode)
        {
            if (copyNode != null)
            {
    ...
    {
        private readonly MemoryFileSystemAsync _fs;
        public DebuggerProxy(MemoryFileSystemAsync fs)
        {
            _fs = fs;
        }
