...
/// <summary>
/// Async counterpart to <see cref = "ZipArchiveFileSystem"/>.
/// </summary>
public class ZipArchiveFileSystemAsync : FileSystemAsync
{
    ...
    private FileSystemEventDispatcherAsync<FileSystemWatcherAsync>? _dispatcher;
    ...
    public ZipArchiveFileSystemAsync(ZipArchive archive, bool isCaseSensitive = false, CompressionLevel compressionLevel = CompressionLevel.NoCompression)
    {
        _archive = archive;
    ...
    public ZipArchiveFileSystemAsync(Stream stream, ZipArchiveMode mode = ZipArchiveMode.Update, bool leaveOpen = false, bool isCaseSensitive = false, CompressionLevel compressionLevel = CompressionLevel.NoCompression) : this(new ZipArchive(stream, mode, leaveOpen: true), isCaseSensitive, compressionLevel)
    {
    ...
    /// <param name = "leaveOpen">True to leave the stream open when <see cref = "ZipArchive"/> is disposed</param>
    /// <param name = "isCaseSensitive">Specifies if entry names should be case sensitive</param>
    public ZipArchiveFileSystemAsync(ZipArchiveMode mode = ZipArchiveMode.Update, bool leaveOpen = false, bool isCaseSensitive = false, CompressionLevel compressionLevel = CompressionLevel.NoCompression) : this(new MemoryStream(), mode, leaveOpen, isCaseSensitive, compressionLevel)
    {
    }
    ...
    private void LoadEntries()
    {
        ...
        _entries = _archive.Entries.ToDictionary(e => new UPath(e.FullName).ToAbsolute(), static e =>
        {
            var lastChar = e.FullName[e.FullName.Length - 1];
            return new InternalZipEntry(e, lastChar is '/' or '\\');
        }, comparer);
        _leadingSlashInArchive = _archive.Entries.Where(e => e.FullName.Length > 0).Any(e => e.FullName.StartsWith("/"));
    }
    ...
    protected override async ValueTask CopyFileAsyncImpl(UPath srcPath, UPath destPath, bool overwrite, CancellationToken cancellationToken)
    {
        ...
        if (await DirectoryExistsAsyncImpl(destPath, cancellationToken: cancellationToken).ConfigureAwait(false))
        {
    ...
    {
        await CreateDirectoryWithoutNotificationAsync(path, cancellationToken: cancellationToken).ConfigureAwait(false);
        TryGetDispatcher()?.RaiseCreated(path);
    }

    private async ValueTask<ZipArchiveEntry> CreateDirectoryWithoutNotificationAsync(UPath path, CancellationToken cancellationToken = default)
    {
        if (await FileExistsAsyncImpl(path, cancellationToken: cancellationToken).ConfigureAwait(false))
        {
            throw FileSystemExceptionHelper.NewDestinationFileExistException(path);
        }

        if (await DirectoryExistsAsyncImpl(path, cancellationToken: cancellationToken).ConfigureAwait(false))
        {
        ...
        if (!parentPath.IsEmpty)
        {
            ...
            {
                await CreateDirectoryAsyncImpl(parentPath.ToString(), cancellationToken: cancellationToken).ConfigureAwait(false);
            }
        }
    ...
    protected override async ValueTask DeleteDirectoryAsyncImpl(UPath path, bool isRecursive, CancellationToken cancellationToken)
    {
        ...
        if (!isRecursive)
        {
            ...
            try
            {
                entries = GetEntriesInDirectory(path.FullName).Take(2).Select(x => x.Value).ToList();
            }
            finally
            {
                _entriesLock.ExitReadLock();
        ...
        try
        {
            entries = GetEntriesInDirectory(path.FullName).Select(x => x.Value).ToList();
            if (entries.Count == 0)
            {
                throw FileSystemExceptionHelper.NewDirectoryNotFoundException(path);
            }
            ...
            foreach (var entry in entries)
            {
                ...
                {
                    throw entry.IsDirectory ? new IOException("Directory is read only") : new UnauthorizedAccessException($"Cannot delete directory that contains readonly entry {entry.Entry.FullName}");
                }
            }
#endif
        }
    ...
    {
        {
            foreach (var item in EnumeratePathsStr(path, "*", searchOption, SearchTarget.Both).Select(p => new FileSystemItemAsync(this, p, p[p.Length - 1] == DirectorySeparator)))
            {
    ...
    {
        {
            foreach (var item in EnumeratePathsStr(path, searchPattern, searchOption, searchTarget).Select(x => new UPath(x)))
            {
                cancellationToken.ThrowIfCancellationRequested();
                yield return item;
                await Task.CompletedTask.ConfigureAwait(false);
    ...
    private IEnumerable<string> EnumeratePathsStr(UPath path, string searchPattern, SearchOption searchOption, SearchTarget searchTarget)
    {
        ...
        try
        {
            var internEntries = path == UPath.Root ? _entries : GetEntriesInDirectory(path.FullName).Where(kv => kv.Key.FullName.Length > path.FullName.Length);
    ...
    protected override async ValueTask<System.Int64> GetFileLengthAsyncImpl(UPath path, CancellationToken cancellationToken)
    {
        ...
        catch (Exception ex) // for some reason entry.Length doesn't work with MemoryStream used in tests
        {
            ...
            using var stream = await OpenFileAsyncImpl(path, FileMode.Open, FileAccess.Read, FileShare.ReadWrite, cancellationToken: cancellationToken).ConfigureAwait(false);
    ...
    {
        return await GetLastWriteTimeAsyncImpl(path, cancellationToken: cancellationToken).ConfigureAwait(false);
    }
    ...
    protected override async ValueTask MoveDirectoryAsyncImpl(UPath srcPath, UPath destPath, CancellationToken cancellationToken)
    {
        ...
        var rootEntry = await CreateDirectoryWithoutNotificationAsync(destPath, cancellationToken: cancellationToken).ConfigureAwait(false);
        foreach (var internalEntry in entries)
        {
            ...
            else
            {
                ...
                if (!isDirectory)
                {
                    ...
                    await entryStream.CopyToAsync(destEntryStream, cancellationToken).ConfigureAwait(false);
                }
            }
    ...
    protected override async ValueTask ReplaceFileAsyncImpl(UPath srcPath, UPath destPath, UPath destBackupPath, bool ignoreMetadataErrors, CancellationToken cancellationToken)
    {
        ...
        if (destEntry != null)
        {
            ...
            if (!destBackupPath.IsEmpty)
            {
                ...
                using var destStream = destEntry.Open();
                await destStream.CopyToAsync(destBackupStream, cancellationToken).ConfigureAwait(false);
            }
        ...
        using (var newStream = newEntry.Open())
        {
            ...
            {
                await sourceStream.CopyToAsync(newStream, cancellationToken).ConfigureAwait(false);
            }
        }
    ...
    {
        var watcher = new FileSystemWatcherAsync(this, path);
        lock (_dispatcherLock)
        {
            _dispatcher ??= new FileSystemEventDispatcherAsync<FileSystemWatcherAsync>(this);
    ...
    private FileSystemEventDispatcherAsync<FileSystemWatcherAsync>? TryGetDispatcher()
    {
        lock (_dispatcherLock)
        {
            return _dispatcher;
    ...
    private sealed class ZipEntryStream : Stream
    {
        ...
        private readonly ZipArchiveFileSystemAsync _fileSystem;
        private readonly Stream _streamImplementation;
        private bool _isDisposed;
        public ZipEntryStream(FileShare share, ZipArchiveFileSystemAsync system, ZipArchiveEntry entry)
        {
            _entry = entry;
            _fileSystem = system;
